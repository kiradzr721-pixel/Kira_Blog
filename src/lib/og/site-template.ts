...
export function getOpenGraphSiteImageTemplate({
	...
	return `<div style="display: flex; flex-direction: column; width: ${widthPx}px; height: ${heightPx}px; padding: 72px; background: #0b0d12; font-family: 'Atkinson Hyperlegible';"><div style="display: flex; flex: 1; flex-direction: column; align-items: center; justify-content: center; gap: 32px;"><div style="display: flex;">${ogLogoSvg({ color: 'white', logoHeightPx: 72 })}</div><div style="display: flex; color: #ffffff; font-size: 84px; font-weight: 700; letter-spacing: -0.03em; line-height: 1;">${escapeHtml(siteName)}</div>${taglineBlock}</div><div style="display: flex; justify-content: center;">${urlBlock}</div></div>`
}
